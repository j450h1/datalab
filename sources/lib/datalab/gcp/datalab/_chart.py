...
def _chart_cell(args, cell):
  ...
  if 'variables' in chart_options:
    ...
    for varname, control in variables.items():
      ...
      # The user should usually the type but we will default to 'text' for strings and 'set' for
      # lists.
      ...
      if type == 'picker':
        ...
        for i, choice in enumerate(choices):
          ...
          # TODO: we may want a 'Submit/Refresh button as we may not want to rerun
        ...
        raise Exception('Unknown control type %s (expected picker, slider, text or set)' % type)
