...
class Schema(list):
  ...
  def from_record(source):
    """
    Infer a BigQuery table schema from a list of fields or a dictionary. The type of the elements
    is used for the types in the schema. For a dict, key names are used for column names while
    for a list, the field names are simply named 'Column1', 'Column2', etc. Note that if
    using a dict you may want to use an OrderedDict to ensure column ordering is deterministic.
    ...
    if isinstance(source, pandas.DataFrame):
      ...
      if len(source) == 0:
        ...
        if all('name' in d and 'type' in d for d in source):
          ...
          bq_schema = Schema._from_list_record(source)
      elif len(source) and isinstance(source[0], list) and \
      ...
      bq_schema = Schema._from_dict_record(source)
    else:
